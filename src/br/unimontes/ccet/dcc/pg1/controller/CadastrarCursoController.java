...
public class CadastrarCursoController {
    public static boolean executa(PainelInterface view){
        ...
        try{
            ...
            int ret = cDao.save(curso);
            if(ret >= 1) return true;
        }
        catch(DAOException e){
            System.err.println("Erro ao tentar cadastrar Curso: " + e.getMessage());
            return false;
        }
        catch(SQLException e2){ 
            System.err.println("Erro ao tentar cadastrar Curso: " + e2.getMessage() + "SQL state: " + e2.getSQLState());
            return false;}
        return false;
    }
